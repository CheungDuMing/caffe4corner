			...
			if (use_rich_annotation) {
				...
				for (int g = 0; g < i; ++g) {
					...
					for (int a = 0; a < g; ++a) {
						...
						if (type == AnnotatedDatum_AnnotationType_PBOX) {
							NormalizedPBox* pbox = anno->mutable_pbox();
							int b = unique_annotation ? a : g;
							pbox->set_ltopx(b*0.1);
							pbox->set_ltopy(b*0.1);
							pbox->set_lbottomx(b*0.1);
							pbox->set_lbottomy(std::min(b*0.1 + 0.2, 1.0));
							pbox->set_rbottomx(std::min(b*0.1 + 0.2, 1.0));
							pbox->set_rbottomy(std::min(b*0.1 + 0.2, 1.0));
							pbox->set_rtopx(std::min(b*0.1 + 0.2, 1.0));
							pbox->set_rtopy(b*0.1);
							pbox->set_difficult(a % 2);
	...
	void TestRead() {
		...
		if (use_rich_annotation_) {
			...
			case AnnotatedDatum_AnnotationType_BBOX:
				...
				break;
			case AnnotatedDatum_AnnotationType_PBOX:
				EXPECT_EQ(blob_top_label_->num(), 1);
				EXPECT_EQ(blob_top_label_->channels(), 1);
				EXPECT_EQ(blob_top_label_->height(), 1);
				EXPECT_EQ(blob_top_label_->width(), 12);
				break;
			default:
			...
			for (int i = 0; i < num_; ++i) {
				if (use_rich_annotation_) {
					if (type_ == AnnotatedDatum_AnnotationType_BBOX) {
						...
						for (int g = 0; g < i; ++g) {
							for (int a = 0; a < g; ++a) {
								EXPECT_EQ(i, label_data[cur_box * 8]);
								EXPECT_EQ(g, label_data[cur_box * 8 + 1]);
								EXPECT_EQ(a, label_data[cur_box * 8 + 2]);
								int b = unique_annotation_ ? a : g;
								for (int p = 3; p < 5; ++p) {
									EXPECT_NEAR(b*0.1, label_data[cur_box * 8 + p], this->eps_);
								}
								for (int p = 5; p < 7; ++p) {
									EXPECT_NEAR(std::min(b*0.1 + 0.2, 1.0),
										label_data[cur_box * 8 + p], this->eps_);
								}
								EXPECT_EQ(a % 2, label_data[cur_box * 8 + 7]);
								cur_box++;
							}
						}
					}
					//**************************************************************************
					if (type_ == AnnotatedDatum_AnnotationType_PBOX) {
						EXPECT_EQ(blob_top_label_->num(), 1);
						EXPECT_EQ(blob_top_label_->channels(), 1);
						EXPECT_EQ(blob_top_label_->height(), PBoxNum(num_));
						EXPECT_EQ(blob_top_label_->width(), 12);
						for (int g = 0; g < i; ++g) {
							for (int a = 0; a < g; ++a) {
								EXPECT_EQ(i, label_data[cur_box * 12]);
								EXPECT_EQ(g, label_data[cur_box * 12 + 1]);
								EXPECT_EQ(a, label_data[cur_box * 12 + 2]);
								int b = unique_annotation_ ? a : g;
								for (int p = 3; p < 6; ++p) {
									EXPECT_NEAR(b*0.1, label_data[cur_box * 12 + p], this->eps_);
								}
								for (int p = 6; p < 9; ++p) {
									EXPECT_NEAR(std::min(b*0.1 + 0.2, 1.0),
										label_data[cur_box * 12 + p], this->eps_);
								}
								EXPECT_NEAR(std::min(b*0.1 + 0.2, 1.0), label_data[cur_box * 12 + 9], this->eps_);
								EXPECT_NEAR(b*0.1, label_data[cur_box * 12 + 10], this->eps_);
								EXPECT_EQ(a % 2, label_data[cur_box * 12 + 11]);
								cur_box++;
							}
						}
					}
			...
			if (use_rich_annotation) {
				...
				if (type == AnnotatedDatum_AnnotationType_PBOX) {
					if (iter <= 1) {
						...
						EXPECT_EQ(blob_top_label_->num(), 1);
						EXPECT_EQ(blob_top_label_->channels(), 1);
						EXPECT_EQ(blob_top_label_->height(), OnePBoxNum(iter));
						EXPECT_EQ(blob_top_label_->width(), 12);
						for (int g = 0; g < iter; ++g) {
							for (int a = 0; a < g; ++a) {
								EXPECT_EQ(i, label_data[cur_box * 12]);
								...
								/*EXPECT_EQ(0, label_data[cur_box * 8]);
								EXPECT_EQ(g, label_data[cur_box * 8 + 1]);
								EXPECT_EQ(a, label_data[cur_box * 8 + 2]);
								int b = unique_annotation ? a : g;
								for (int p = 3; p < 5; ++p) {
...
TYPED_TEST(AnnotatedDataLayerTest, TestReadLevelDB) {
	...
	type = AnnotatedDatum_AnnotationType_PBOX;
	for (int p = 0; p < kNumChoices; ++p) {
		bool unique_pixel = kBoolChoices[p];
		for (int r = 0; r < kNumChoices; ++r) {
			bool use_rich_annotation = kBoolChoices[r];
