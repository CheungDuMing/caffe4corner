    ...
    if (v1.first == "object") {
      ...
      BOOST_FOREACH(ptree::value_type &v2, object.get_child("")) {
        ...
        if (v2.first == "name") {
          ...
          //LOG(INFO) << anno_datum->annotation_group_size();
		...
		//****************************************
		//use your type
		else if (v2.first == "pbndbox") 
		{
          //LOG(INFO) << anno_datum->annotation_group_size();
			int ltopx = pt2.get("ltopx", 0);
			int ltopy= pt2.get("ltopy", 0);
			int lbottomx = pt2.get("lbottomx", 0);
			int lbottomy = pt2.get("lbottomy", 0);
			int rbottomx = pt2.get("rbottomx", 0);
			int rbottomy = pt2.get("rbottomy", 0);
			int rtopx = pt2.get("rtopx", 0);
			int rtopy = pt2.get("rtopy", 0);
			//*********************************************
			//your log method
			 CHECK_NOTNULL(anno);
			/*LOG_IF(WARNING, xmin > width) << labelfile <<
			...
			" bounding box irregular.";
			*/
			// Store the normalized polygan bounding box.
			//*********************************************************
			//can not use bbox type anymore
      //std::cout << "import label successfully!!!!!!" << std::endl;
			NormalizedPBox* pbox = anno->mutable_pbox();
			pbox->set_ltopx(static_cast<float>(ltopx) / width);
			pbox->set_ltopy(static_cast<float>(ltopy) / height);
			pbox->set_lbottomx(static_cast<float>(lbottomx) / width);
			pbox->set_lbottomy(static_cast<float>(lbottomy) / height);
			pbox->set_rbottomx(static_cast<float>(rbottomx) / width);
			pbox->set_rbottomy(static_cast<float>(rbottomy) / height);
			pbox->set_rtopx(static_cast<float>(rtopx) / width);
			pbox->set_rtopy(static_cast<float>(rtopy) / height);
