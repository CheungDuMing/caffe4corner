...
void UpdatePBoxByResizePolicy(const ResizeParameter& param,
	const int old_width, const int old_height,
	NormalizedPBox* pbox) {
	float new_height = param.height();
	float new_width = param.width();
	...
	float ltopx = pbox->ltopx() * old_width;
	float lbottomx = pbox->lbottomx() * old_width;
	float rtopx = pbox->rtopx() * old_width;
	float rbottomx = pbox->rbottomx() * old_width;
	float ltopy = pbox->ltopy() * old_height;
	float lbottomy = pbox->lbottomy() * old_height;
	float rtopy = pbox->rtopy() * old_height;
	float rbottomy = pbox->rbottomy() * old_height;
	//float x_min = bbox->xmin() * old_width;
	//float y_min = bbox->ymin() * old_height;
	//float x_max = bbox->xmax() * old_width;
	//float y_max = bbox->ymax() * old_height;
	...
	switch (param.resize_mode()) {
	case ResizeParameter_Resize_mode_WARP:
		ltopx = std::min(new_width, std::max(0.f, ltopx * new_width / old_width));
		lbottomx = std::min(new_width, std::max(0.f, lbottomx * new_width / old_width));
		rtopx = std::min(new_width, std::max(0.f, rtopx * new_width / old_width));
		rbottomx = std::min(new_width, std::max(0.f, rbottomx * new_width / old_width));

		ltopy = std::min(new_height, std::max(0.f, ltopy * new_height / old_height));
		lbottomy = std::min(new_height, std::max(0.f, lbottomy * new_height / old_height));;
		rtopy = std::min(new_height, std::max(0.f, rtopy * new_height / old_height));;
		rbottomy = std::min(new_height, std::max(0.f, rbottomy * new_height / old_height));;
		/*x_min = std::max(0.f, x_min * new_width / old_width);
		x_max = std::min(new_width, x_max * new_width / old_width);
		y_min = std::max(0.f, y_min * new_height / old_height);
		y_max = std::min(new_height, y_max * new_height / old_height);*/
	...
	pbox->set_ltopx(ltopx / new_width);
	pbox->set_lbottomx(lbottomx / new_width);
	pbox->set_rtopx(rtopx / new_width);
	pbox->set_rbottomx(rbottomx / new_width);
	pbox->set_ltopy(ltopy / new_height);
	pbox->set_lbottomy(lbottomy / new_height);
	pbox->set_rtopy(rtopy / new_height);
	pbox->set_rbottomy(rbottomy / new_height);
}
