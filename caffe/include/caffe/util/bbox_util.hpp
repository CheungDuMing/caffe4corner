...
typedef PriorPBoxParameter_CodeType PCodeType;
typedef MultiPBoxLossParameter_MatchType PMatchType;
typedef MultiPBoxLossParameter_LocLossType PLocLossType;
typedef MultiPBoxLossParameter_ConfLossType PConfLossType;
typedef MultiPBoxLossParameter_MiningType PMiningType;

typedef map<int, vector<NormalizedBBox> > LabelBBox;
typedef map<int, vector<NormalizedPBox> > LabelPBox;
...
// Check if a pbox is cross boundary or not.
...
// Compute pbox size.
float PBoxSize(const NormalizedPBox& pbox, const bool normalized = true);

template <typename Dtype>
Dtype BBoxSize(const Dtype* bbox, const bool normalized = true);
template <typename Dtype>
Dtype PBoxSize(const Dtype* pbox, const bool normalized = true);
...
template <typename Dtype>
Dtype JaccardOverlapPB(const Dtype* bbox, const Dtype* pbox);
...
//***********************************************************************
bool MeetEmitConstraint(const NormalizedBBox& src_bbox,
    const NormalizedPBox& pbox, const EmitConstraint& emit_constraint);
...
//*****************************************************************
//pbox version
// Decode a pbox according to a prior pbox.
void DecodePBox(const NormalizedPBox& prior_pbox,
	const vector<float>& prior_variance, const PCodeType code_type,
	const bool variance_encoded_in_target, const bool clip_pbox,
	const NormalizedPBox& pbox, NormalizedPBox* decode_pbox);

// Decode a set of bboxes according to a set of prior bboxes.
void DecodePBoxes(const vector<NormalizedPBox>& prior_pboxes,
    const vector<vector<float> >& prior_variances,
...
//pbox version
void DecodePBoxesAll(const vector<LabelPBox>& all_loc_pred,
...
//**************************************************************
// Match prediction pboxes with ground truth pboxes.
void MatchPBox(const vector<NormalizedPBox>& gt,
...
//*********************************************************************************
// Find matches between prediction pboxes and ground truth pboxes.
//    all_loc_preds: stores the location prediction, where each item contains
//      location prediction for an image.
//    all_gt_bboxes: stores ground truth bboxes for the batch.
//    prior_bboxes: stores all the prior bboxes in the format of NormalizedBBox.
...
// Retrieve polygon bounding box ground truth from gt_data.
//    gt_data: 1 x 1 x num_gt x 11 blob.
//    num_gt: the number of ground truth.
//    background_label_id: the label for background class which is used to do
//      santity check so that no ground truth contains it.
//    all_gt_bboxes: stores ground truth for each image. Label of each bbox is
...
template <typename Dtype>
__host__ __device__ Dtype JaccardOverlapPBGPU(const Dtype* bbox,
	const Dtype* pbox);
