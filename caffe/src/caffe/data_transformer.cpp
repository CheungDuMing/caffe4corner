...
void DataTransformer<Dtype>::TransformAnnotation(
  ...
  //***************************************************************************
  //pbox version
  else if (anno_datum.type() == AnnotatedDatum_AnnotationType_PBOX) {
	  // Go through each AnnotationGroup.
	  for (int g = 0; g < anno_datum.annotation_group_size(); ++g) {
		  const AnnotationGroup& anno_group = anno_datum.annotation_group(g);
		  AnnotationGroup transformed_anno_group;
        ...
        //LOG(INFO) << pbox.ltopx() << " " << pbox.rtopx() <<" " << pbox.ltopy() << " " << pbox.rtopy();
        //LOG(INFO) << pbox.lbottomx() << " " << pbox.rbottomx() <<" " << pbox.lbottomy() << " " << pbox.rbottomy();
        //LOG(INFO) << pbox.has_size();
        //LOG(INFO) << PBoxSize(pbox);
			  // Adjust polygan bounding box annotation.
			  NormalizedPBox resize_pbox = pbox;
			  if (do_resize && param_.has_resize_param()) {
				  CHECK_GT(img_height, 0);
				  CHECK_GT(img_width, 0);
				  // *******************************************************************
				  //only support the WARP resize mode
				  UpdatePBoxByResizePolicy(param_.resize_param(), img_width, img_height,
					  &resize_pbox);
			  }
			  //******************************************************
			  if (param_.has_emit_constraint() &&
				  !MeetEmitConstraint(crop_bbox, resize_pbox,
					  param_.emit_constraint())) {
				  continue;
			  }
			  NormalizedPBox proj_pbox;
			  if (ProjectPBox(crop_bbox, resize_pbox, &proj_pbox)) {
				  has_valid_annotation = true;
				  Annotation* transformed_anno =
					  transformed_anno_group.add_annotation();
				  transformed_anno->set_instance_id(anno.instance_id());
				  ...
				  if (do_resize && param_.has_resize_param()) {
					  ExtrapolatePBox(param_.resize_param(), img_height, img_width,
						  crop_bbox, transformed_pbox);
				  }
			  }
		  }
		  // Save for output.
...
void DataTransformer<Dtype>::CropImage(const AnnotatedDatum& anno_datum,
  ...
  //LOG(INFO) << BBoxSize(crop_bbox);
  TransformAnnotation(anno_datum, do_resize, crop_bbox, do_mirror,
                      cropped_anno_datum->mutable_annotation_group());
  //LOG(FATAL) << cropped_anno_datum->annotation_group_size();
}
...
void DataTransformer<Dtype>::ExpandImage(const AnnotatedDatum& anno_datum,
  ...
  TransformAnnotation(anno_datum, do_resize, expand_bbox, do_mirror,
   ...
   // LOG(FATAL) << expanded_anno_datum->annotation_group_size();
}
