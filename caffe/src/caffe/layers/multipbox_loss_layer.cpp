...
#include "caffe/layers/multipbox_loss_layer.hpp"
#include "caffe/util/math_functions.hpp"

namespace caffe {

	template <typename Dtype>
	void MultiPBoxLossLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
		...
		const MultiPBoxLossParameter& multipbox_loss_param =
			this->layer_param_.multipbox_loss_param();
		multipbox_loss_param_ = this->layer_param_.multipbox_loss_param();

		num_ = bottom[0]->num();
		num_priors_ = bottom[2]->height() / 8;;
		// Get other parameters.
		CHECK(multipbox_loss_param.has_num_classes()) << "Must provide num_classes.";
		num_classes_ = multipbox_loss_param.num_classes();
		CHECK_GE(num_classes_, 1) << "num_classes should not be less than 1.";
		share_location_ = multipbox_loss_param.share_location();
		loc_classes_ = share_location_ ? 1 : num_classes_;
		background_label_id_ = multipbox_loss_param.background_label_id();
		use_difficult_gt_ = multipbox_loss_param.use_difficult_gt();
		mining_type_ = multipbox_loss_param.mining_type();
		if (multipbox_loss_param.has_do_neg_mining()) {
			LOG(WARNING) << "do_neg_mining is deprecated, use mining_type instead.";
			do_neg_mining_ = multipbox_loss_param.do_neg_mining();
			CHECK_EQ(do_neg_mining_,
				mining_type_ != MultiPBoxLossParameter_MiningType_NONE);
		}
		do_neg_mining_ = mining_type_ != MultiPBoxLossParameter_MiningType_NONE;
		...
		loc_weight_ = multipbox_loss_param.loc_weight();
		loc_loss_type_ = multipbox_loss_param.loc_loss_type();
		// fake shape.
		...
		if (loc_loss_type_ == MultiPBoxLossParameter_LocLossType_L2) {
		...
		else if (loc_loss_type_ == MultiPBoxLossParameter_LocLossType_SMOOTH_L1) {
			LayerParameter layer_param;
		...
		//*********************************************************
		conf_loss_type_ = multipbox_loss_param.conf_loss_type();
		...
		if (conf_loss_type_ == MultiPBoxLossParameter_ConfLossType_SOFTMAX) {
		...
		else if (conf_loss_type_ == MultiPBoxLossParameter_ConfLossType_LOGISTIC) {
			LayerParameter layer_param;
	...
	template <typename Dtype>
	void MultiPBoxLossLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
		const vector<Blob<Dtype>*>& top) {
		LossLayer<Dtype>::Reshape(bottom, top);
		num_ = bottom[0]->num();
		num_priors_ = bottom[2]->height() / 8;
		//LOG(INFO) << num_priors_;
	...
	template <typename Dtype>
	void MultiPBoxLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
		const vector<Blob<Dtype>*>& top) {
		...
		GetPboxGroundTruth(gt_data, num_gt_, background_label_id_, use_difficult_gt_,
			&all_gt_pboxes);
		...
		GetPboxLocPredictions(loc_data, num_, num_priors_, loc_classes_, share_location_,
			&all_loc_preds);

		// Find matches between source pboxes and ground truth pboxes.
		//***********************************************************************
		vector<map<int, vector<float> > > all_match_overlaps;
		FindPboxMatches(all_loc_preds, all_gt_pboxes, prior_pboxes, prior_variances,
			multipbox_loss_param_, &all_match_overlaps, &all_match_indices_);
			//LOG(INFO) << all_match_indices_.size() << " " << all_match_indices_[0].begin()->second.size();

		num_matches_ = 0;
		int num_negs = 0;
		// Sample hard negative (and positive) examples based on mining type.
		//***********************************************************************
		MineHardExamples(*bottom[1], all_loc_preds, all_gt_pboxes, prior_pboxes,
			prior_variances, all_match_overlaps, multipbox_loss_param_,
			&num_matches_, &num_negs, &all_match_indices_,
			&all_neg_indices_);
		//LOG(INFO) << "num_match: " << num_matches_;
		...
		if (num_matches_ >= 1) {
			...
			//*******************************************************************
			EncodePboxLocPrediction(all_loc_preds, all_gt_pboxes, all_match_indices_,
				prior_pboxes, prior_variances, multipbox_loss_param_,
		...
		if (num_conf_ >= 1) {
			...
			EncodeConfPrediction(conf_data, num_, num_priors_, multipbox_loss_param_,
				all_match_indices_, all_neg_indices_, all_gt_pboxes,
	...
	template <typename Dtype>
	void MultiPBoxLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
		...
		if (propagate_down[1]) {
			...
			if (num_conf_ >= 1) {
				...
				if (do_neg_mining_) {
					int count = 0;
					for (int i = 0; i < num_; ++i) {
						// Copy matched (positive) pboxes scores' diff.
						...
						// Copy negative pboxes scores' diff.
	...
	INSTANTIATE_CLASS(MultiPBoxLossLayer);
	REGISTER_LAYER_CLASS(MultiPBoxLoss);
