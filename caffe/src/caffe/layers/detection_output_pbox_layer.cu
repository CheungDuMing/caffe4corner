...
void DetectionOutputPboxLayer<Dtype>::Forward_gpu(
  ...
  // Decode predictions.
  Dtype* pbox_data = pbox_preds_.mutable_gpu_data();
  const int loc_count = pbox_preds_.count();
  const bool clip_pbox = false;
  DecodePBoxesGPU<Dtype>(loc_count, loc_data, prior_data, code_type_,
      variance_encoded_in_target_, num_priors_, share_location_,
      num_loc_classes_, background_label_id_, clip_pbox, pbox_data);
  // Retrieve all decoded location predictions.
  const Dtype* pbox_cpu_data;
  if (!share_location_) {
    Dtype* pbox_permute_data = pbox_permute_.mutable_gpu_data();
    //********************
	PermuteDataGPU<Dtype>(loc_count, pbox_data, num_loc_classes_, num_priors_,
        8, pbox_permute_data);
    pbox_cpu_data = pbox_permute_.cpu_data();
  } else {
    pbox_cpu_data = pbox_preds_.cpu_data();
  }
  ...
  for (int i = 0; i < num; ++i) {
    ...
    for (int c = 0; c < num_classes_; ++c) {
      ...
      const Dtype* cur_pbox_data = pbox_cpu_data + pbox_idx;
      if (!share_location_) {
  ...
  for (int i = 0; i < num; ++i) {
    ...
    for (map<int, vector<int> >::iterator it = all_indices[i].begin();
      ...
      const Dtype* cur_pbox_data = pbox_cpu_data + pbox_idx;
      if (!share_location_) {
        cur_pbox_data += label * num_priors_ * 8;
      }
      for (int j = 0; j < indices.size(); ++j) {
        int idx = indices[j];
        top_data[count * 11] = i;
        top_data[count * 11 + 1] = label;
        top_data[count * 11 + 2] = cur_conf_data[idx];
        for (int k = 0; k < 8; ++k) {
          top_data[count * 11 + 3 + k] = cur_pbox_data[idx * 8 + k];
        }
        if (need_save_) {
          // Generate output bbox.
          NormalizedPBox pbox;
		  pbox.set_ltopx(top_data[count * 11 + 3]);
		  pbox.set_ltopy(top_data[count * 11 + 4]);
		  pbox.set_lbottomx(top_data[count * 11 + 5]);
		  pbox.set_lbottomy(top_data[count * 11 + 6]);
		  pbox.set_rbottomx(top_data[count * 11 + 7]);
		  pbox.set_rbottomy(top_data[count * 11 + 8]);
		  pbox.set_rtopx(top_data[count * 11 + 9]);
		  pbox.set_rtopy(top_data[count * 11 + 10]);
         /*  bbox.set_xmin(top_data[count * 11 + 3]);
          bbox.set_ymin(top_data[count * 11 + 4]);
          bbox.set_xmax(top_data[count * 11 + 5]);
          bbox.set_ymax(top_data[count * 11 + 6]); */
...
INSTANTIATE_LAYER_GPU_FUNCS(DetectionOutputPboxLayer);
